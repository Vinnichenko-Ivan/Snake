...
class Snake:
    ...
    def eat(self, newDirection = direction):
        if(abs(self.direction - newDirection) != 2 and newDirection != -1):
            self.direction = newDirection
        self.size += 1
        self.segments.append([self.segments[len(snake.segments) - 1][0],self.segments[len(snake.segments) - 1][1]])
        if (self.direction == 0):
            self.segments[len(snake.segments) - 1][1] -= 1
        if (self.direction == 1):
            self.segments[len(snake.segments) - 1][0] += 1
    ...
    def testCollision(this):
        for i in range(0, len(snake.segments) - 1):
            if (snake.segments[i][0] == snake.segments[len(snake.segments) - 1][0] and snake.segments[i][1] ==
                    snake.segments[len(snake.segments) - 1][1]):
                return True
        return False
#--------------------------------------------------------------------------------------------------------------
...
def drawSnake(screen,snake):
    for i in range(0,len(snake.segments) - 1):
        pygame.draw.rect(screen, SNAKECOLOR, (snake.segments[i][0] * CELLSIZEXPX + 1, snake.segments[i][1] * CELLSIZEXPY + 1, CELLSIZEXPX - 1, CELLSIZEXPY - 1))
    pygame.draw.rect(screen, SNAKECOLOR, (snake.segments[len(snake.segments) - 1][0] * CELLSIZEXPX + 1, snake.segments[len(snake.segments) - 1][1] * CELLSIZEXPY + 1, CELLSIZEXPX - 1, CELLSIZEXPY - 1))

def drawFood(screen,food):
    pygame.draw.rect(screen, FOODCOLOR, (food.x * CELLSIZEXPX + 1, food.y * CELLSIZEXPY + 1, CELLSIZEXPX - 1, CELLSIZEXPY - 1))

def testEating(food,snake):
    if(food.x == snake.segments[len(snake.segments) - 1][0] and food.y == snake.segments[len(snake.segments) - 1][1]):
        return True
    return False
